...
class RegistrationForm(FlaskForm):
    ...
    password = PasswordField('Password', validators=[DataRequired(), 
                                                         ...
                                                         Regexp(r'.*[^a-zA-Z0-9].*', message="Password must contain at least one symbol.")])
    ...
    def validate_password(self, password):
        # Basic password strength check (already covered by regex validators)
        # This method is primarily for future, more complex checks if needed.
        pass
...
class TransferForm(FlaskForm):
    ...
    recipient_account = StringField('Recipient Account Number', validators=[Optional(), Length(min=10, max=10, message="Account number must be 10 digits long."), Regexp(r'^\d{10}$', message="Account number must contain only digits.")])
    ...
    def validate(self, extra_validators=None):
        ...
        # Add a placeholder for maximum amount check
        MAX_TRANSFER_AMOUNT = 10000.00 # Example limit, should be configurable
        if self.amount.data is not None and self.amount.data > MAX_TRANSFER_AMOUNT:
            self.amount.errors.append(f'Transfer amount exceeds the maximum limit of ₱{MAX_TRANSFER_AMOUNT:.2f}')
            return False
...
class DepositForm(FlaskForm):
    account_number = StringField('Account Number', validators=[DataRequired(), Length(min=10, max=10, message="Account number must be 10 digits long."), Regexp(r'^\d{10}$', message="Account number must contain only digits.")])
    amount = FloatField('Amount', validators=[DataRequired(), NumberRange(min=0.01, message="Amount must be greater than 0")])
    ...
    def validate(self, extra_validators=None):
        ...
        # Add a placeholder for maximum amount check
        MAX_DEPOSIT_AMOUNT = 50000.00 # Example limit, should be configurable
        if self.amount.data is not None and self.amount.data > MAX_DEPOSIT_AMOUNT:
            self.amount.errors.append(f'Deposit amount exceeds the maximum limit of ₱{MAX_DEPOSIT_AMOUNT:.2f}')
            return False
