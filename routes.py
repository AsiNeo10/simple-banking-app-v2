...
def activate_user(user_id):
    ...
    transaction = Transaction(
        ...
        details=f'Account {user.username} activated by {current_user.username}'
    )
...
def deactivate_user(user_id):
    ...
    transaction = Transaction(
        ...
        receiver_id=user.id,
        amount=0.0,
        transaction_type='account_deactivation',
        timestamp=datetime.datetime.utcnow(),
        details=f'Account {user.username} deactivated by {current_user.username}'
    )
    db.session.add(transaction)
    db.session.commit()
...
def edit_user(user_id):
    ...
    if form.validate_on_submit():
        ...
        # Update address codes if names were selected
        user.region_code = form.region_name.data if form.region_name.data else user.region_code
        user.province_code = form.province_name.data if form.province_name.data else user.province_code
        user.city_code = form.city_name.data if form.city_name.data else user.city_code
        user.barangay_code = form.barangay_name.data if form.barangay_name.data else user.barangay_code

        # Update address names based on selected codes (optional, could also lookup)
        # For simplicity, we'll just use the selected names from the form if available
        user.region_name = form.region_name.raw_data[0][1] if form.region_name.raw_data and form.region_name.data else user.region_name # raw_data[0][1] gets the selected choice text
        user.province_name = form.province_name.raw_data[0][1] if form.province_name.raw_data and form.province_name.data else user.province_name
        user.city_name = form.city_name.raw_data[0][1] if form.city_name.raw_data and form.city_name.data else user.city_name
        user.barangay_name = form.barangay_name.raw_data[0][1] if form.barangay_name.raw_data and form.barangay_name.data else user.barangay_name
